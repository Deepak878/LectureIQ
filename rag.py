...
def load_groq_client():
    """loading groq client using api key"""
    api_key = os.getenv("GROQ_API_KEY")
    if not api_key:
        raise ValueError(" error: GROQ_API_KEY not set in .env ")
    client = Groq(api_key=api_key)
    # print(f"groq is ready, model: {GROQ_MODEL}")
    return client


def build_context_from_segments(segments):
    """formatting the retrieved transcript segments into readable context block for llm"""
    context_parts = []
    for seg in segments:
...
def build_system_prompt(metadata=None):
    """building system prompt, putting lecture metadata if provided"""
    ...
    # putting course context if given
    if metadata:
        context_lines = []
...
def ask_lecture(question, context_segments, groq_client, metadata=None):
    """main rag function; takes question, retrieved segments and  returns llm answer"""

    context_text = build_context_from_segments(context_segments)

    system_prompt = build_system_prompt(metadata)

    user_message = f"Lecture transcript context:\n{context_text}\n\nQuestion: {question}"
    ...
    response = groq_client.chat.completions.create(
        ...
        temperature=0.1,   # lower = more grounded, less creative/hallucination
        max_tokens=512,
    )
